...
<!--      分类-->
      <van-field
          ...
          label="简介"
          placeholder="简介"
          :rules="[{ required: true, message: '请填写简介' },{ min: 1, max: 300, message: '故事简介长度必须介于 1 和 300 之间'}]"
      />
      <van-field
          ...
          label="说明"
          placeholder="说明"
          :rules="[{ required: true, message: '请填写说明' }, { min: 10, max:1000, message: '故事描述长度必须介于 10 和 1000 之间'}]"
      />
    </van-cell-group>
...
import {  addWorld,updateImage } from "@/api/admin/world";

// import {  updateImage } from "@/api/tool/common";
...
const onSubmit = (values) => {
  ...
  addWorld(form.value).then(response => {
    ...
    router.push("/admin/world?wid="+response.data.id+"&wname="+response.data.name);
  })
};
