<template>
  ...
  <van-list>
    <van-swipe-cell v-for="reel in chapterList">
      ...
      <template #right>
        <van-button square type="primary" text="查看章节" @click="handleSeeChapter(reel)" />
        <van-button square type="success" text="修改名称" @click="onClickUpdateShow(reel.title)" />
...
const onClickUpdateShow = (title:string) => {
...
function handleSeeChapter(reel){
  router.push("/admin/storyChapter?sid="+reel.sid+"&sname="+sname.value+"&scid="+reel.id+"&scname="+reel.title);
}
/** 查询元素列表 */
function getList() {
