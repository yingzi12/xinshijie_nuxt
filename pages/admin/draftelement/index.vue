<template>
  ...
  <div >
    ...
    <div class="center" style="height: 80px;">
      <van-button v-if="element.status == 7"  @click="handleIssue">发布</van-button>
      <van-button v-if="element.status == 7"  @click="handleDelete" >删除</van-button>
    </div>
  </div>
</template>
...
import {  getDraftDetails,delDraft,issue } from "@/api/admin/draftElement";
...
/** 查询元素详细 */
function getElement() {
  getDraftDetails(wid.value,deid.value,1).then(response => {
...
function handleDelete (){
  Dialog.confirm({
    title: '删除章节',
    message:
        '是否确认删除章节名称为"' + element.value.title + '"的草稿数据？'
  }).then(() => {
    delDraft(wid.value,deid.value).then(response => {
          Dialog.alert({message:'删除成功'})
          router.push("/admin/draftElementList?wid="+wid.value);
        }
    )
  })
      .catch(() => {
